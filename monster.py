...
class Monster:
    ...
    def __init__(self, name, types, level, base_stats, moves):
        ...
        # 次のレベルアップに必要な総経験値
        self.exp_to_next_level = get_exp_for_level(level + 1)
    ...
    def level_up(self):
        ...
        self.level += 1
        self.exp_to_next_level = get_exp_for_level(self.level + 1)
        
        # ステータスを再計算
        old_stats = (self.max_hp, self.attack, self.defense, self.sp_attack, self.sp_defense, self.speed)
        self._calculate_stats()
        new_stats = (self.max_hp, self.attack, self.defense, self.sp_attack, self.sp_defense, self.speed)
